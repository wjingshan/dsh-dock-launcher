...
| ![启动中](docs/icon-starting.gif) | **启动中** | 白色滑块左右往复（胶囊通电变绿） |
| ![启动失败](docs/icon-failed.png) | **启动失败** | 关闭态 · 圆钮上是红色 ✕ |
...
> 八张图放在**同一列**、而不是排成一行：GitHub 的 `max-width: 100%` 会把图片压缩到所在单元格的宽度。
> 横排时每列宽度都不同（实测 121～235px），每张图就被压成不同尺寸，看起来"大小不一"；放在同一列则
> 共用同一个列宽，无论视口多宽，八张图的显示尺寸必然完全一致。

> 点下图标**立刻**进入「启动中」：关闭态红胶囊上绿色从左端扫满（0.3 秒通电，前缘带一道柔光），此后
> 胶囊保持绿色，**白色滑块在整只胶囊里左↔右往复**（余弦，两端速度为零；一个来回 1.1 秒）——「开关已经
> 通了电，还在等服务就绪」。服务就绪时滑块从当前位置**平滑滑到右端停住**（0.25 秒归位），落定那一帧与
> 「运行 · 空闲」图标**逐像素一致**，无缝接管。**启动失败**（未找到 dsh / 起不来 / 120 秒无响应）则是
> 绿色从右端**退去**、滑块滑回左端，并在中间那颗圆钮上弹出**红色 ✕**（0.55 秒入场，之后静止），同时
> 弹出原因说明。
...
2. **API keys（含 `DEEPSEEK_API_KEY`）**：右键菜单 **「编辑 API keys…」** 写入 `$DSH_HOME/.env`（缺省 `~/.dsh/.env`，dsh 启动时自动读取并注入所有插件）；也可在 `~/.zshrc` 里 `export`，或在 dsh 网页的 Models 页保存。否则 dsh 发消息会报 `no API key`，依赖第三方 API 的插件（例如鲸鱼娘 Galgame 的升级 CG 需要 `DASHSCOPE_API_KEY`）会报缺少 key；
...
- **点下就有反馈**：点击图标后**立即**切到「启动中」动画（绿色通电 + 白色滑块左右往复），页面出现的这几秒不再是一片空白；成功时滑块归位、无缝变成「运行 · 空闲」，失败则绿电退去并在圆钮上留下**红色 ✕**，一眼就能区分「还在启动」和「启动失败」。
...
- **状态图标（6 态）**：
  - `关闭`（红，滑块左）= 服务已停止
  - `启动中`：点下图标立刻播放的**通电 + 滑块左右往复**动画（见上）
  - `启动失败`：关闭态 + 圆钮上的**红色 ✕**（入场：绿电退去 + 滑块归左 + ✕ 弹入，之后静止），菜单变为「重新启动 DSH 服务」
  - `运行 · 空闲`（绿，滑块右）
...
- **API keys 通用入口（右键菜单「编辑 API keys…」）**：直接打开 dsh 自己的用户级环境文件 `$DSH_HOME/.env`（缺省 `~/.dsh/.env`，不存在时按 `0600` 写入带注释的模板）。dsh 启动时读取该文件并注入**所有**插件与工具进程，所以以后接入任何新 API 只要加一行 `KEY=VALUE` —— 本 App 不需要跟着改版，也不认识任何具体的 key 名。保存后需要重启 DSH 服务生效（弹窗里可直接点「重启 DSH 服务」）。
- **兜底的 shell 解析**：GUI 启动的进程不读 `.zshrc`，本 App 仍会解析 `~/.zshenv` / `.zprofile` / `.zshrc` / `.bash_profile` / `.bashrc` / `.profile` 中形如 `*_API_KEY` / `*_TOKEN` / `*_SECRET` / `*_PASSWORD` / `*_CREDENTIAL` 的变量（含 `*_ACCESS_KEY_ID`），并排除 `*_PATH` / `*_FILE` / `*_DIR` / `*_URL` 这类「名字像 key 其实是路径」的项。已存在的进程环境变量优先；`$DSH_HOME/.env` 由 dsh 自己读取，本 App **不重复注入**，以免两套解析器对同一个值产生分歧。整条链路只读、只写变量名进日志，不记录任何值、不外传。
...
本 App 的组合目前没有第二个项目同时具备：**程序坞图标本身就是状态显示与唯一入口**（左键回页面 / 右键出菜单），服务未运行时左键直接拉起服务；图标实时区分 `关闭 / 启动中 / 启动失败 / 空闲 / 进行中 / 待关注` 六态（点下图标就有拨动动画，失败直接留一个红 ✕）；任务完成由**真实的 goal 结束事件**驱动，多会话并行不漏检。

## 使用

1. 把 `DeepSeek Harness 开关.app` 拖进「应用程序」，双击打开（或拖到程序坞固定）。
2. App 启动时**如实显示服务状态**（未运行则显示红色「关闭」），**不会自动拉起 dsh**。
3. **左键**点程序坞图标 = 启动服务并打开 dsh 页面（已运行则回到页面）。点下后会立刻看到**白色滑块左右往复**的「启动中」动画；若启动失败，绿电退去、圆钮上出现**红色 ✕**，右键菜单随之变为「重新启动 DSH 服务」。
4. **右键**点程序坞图标 = 打开操作菜单（停止服务需二次确认）。
5. 菜单栏小图标点击可打开同一套菜单。

> 提示：提醒共有**四条通道**——Dock 图标动画、程序坞弹跳、提示音、系统通知。**系统通知是可选的**：不需要横幅的话，右键菜单里选「关闭系统通知」即可，此时连授权框都不会弹（授权只在首次真正要投递通知时才申请）。

## 权限说明

本 App 只涉及**两个**系统权限，而且**两个都是可选的**——不授权也能正常用。

| 权限 | 用途 | 不授权会怎样 |
|---|---|---|
| **系统通知** | 提醒横幅（四条提醒通道之一） | 其余三条照常：Dock 图标动画、程序坞弹跳、提示音。右键菜单可关掉这项 |
| **自动化（AppleEvents）** | **只访问浏览器**：读活动标签页地址，精确判断你已回到 `127.0.0.1:3080`；点击图标时切回已打开的标签而非新开 | 退化为「浏览器在前台即视为已回到界面」；点击图标改为激活浏览器 + 打开地址（可能多开一个标签） |

**没有申请、也没有使用的权限**：辅助功能、输入监控、屏幕录制、完全磁盘访问、文件与文件夹（桌面 / 文稿 / 下载）、麦克风、摄像头、定位、通讯录、日历、照片、登录项。

其中值得一提的是：「鼠标移到程序坞就停提示音」用的是**定时轮询鼠标位置**（`NSEvent.mouseLocation`），不是全局事件监听，所以确实**不需要辅助功能或输入监控**权限。

另外，App 未启用沙盒（entitlements 为空、ad-hoc 签名）：这既是它能自由调用 homebrew 里的 `dsh` / `zstd`、读写 `~/.dsh` 的前提，也意味着它没有公证——首次打开需要靠 `install.sh` 去掉 quarantine 标记。
...
│   ├── ServiceManager.swift# 启动/停止/端口探测、通用 API 凭据管线（$DSH_HOME/.env + shell 兜底）
