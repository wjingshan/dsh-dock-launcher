...
| 红 · 滑块在左 | 绿 · 滑块在右 | 多色极光流动 | 边缘内发光 + 白色流光（绿） | 边缘内发光 + 白色流光（橙） | 边缘内发光 + 白色流光（紫） |

> 最后一格是**动图**：dsh 停下来等你选分支（`ask_user_question`）或审批计划（`exit_plan_mode`）时就进入这个紫色的「等你选择」态，直到你回到页面或点「知道了」。
...
- **状态图标（4 态）**：
  ...
  - `待关注提醒`：边缘向内呼吸发光 + 边缘内侧 2px 白色流光（沿边环绕）；完成=绿、需确认=橙、**等你选择=紫**
- **动态提醒**：任务完成 / 需要确认 / **等你选择**（选项分支、计划审批）时，图标先弹跳 4 次 + 通知 + 音效（完成=`Glass`、需确认=`Purr`、等你选择=`Ping`），随后转为**安静的持续发光**；等你**回到 dsh 页面**、点图标或菜单「知道了」即停止（不会周期性乱跳）。
