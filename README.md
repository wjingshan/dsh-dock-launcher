...
| 关闭 | 运行 · 空闲 | 任务进行中 |
|:--:|:--:|:--:|
| ![关闭](docs/icon-off.png) | ![运行](docs/icon-running.png) | ![进行中](docs/icon-busy.gif) |
| 红 · 滑块在左 | 绿 · 滑块在右 | 多色极光流动 |

**提醒状态**（动图）

| 任务完成 | 需要确认 / 等你选择 · 单选 | 等你选择 · 多选 |
|:--:|:--:|:--:|
| ![完成](docs/icon-remind-complete.gif) | ![需要确认/单选](docs/icon-remind-question.gif) | ![多选](docs/icon-remind-multi.gif) |
| 边缘内发光 + 白色流光（绿） | 问号变形动画 | 对勾淡化叠加 |

> 拆成两张三列表不是随意的：GitHub 的 `max-width: 100%` 会把图片**压缩到所在单元格的宽度**，
> 六列表时每列只有 ~86px，各列宽度又因表头长短而不同，于是每张图被压成的尺寸都不一样（看起来"大小不一"）。
> 三列时单元宽度约 200px，超过图片 128px 的固有宽度，七张图就都按原尺寸显示了。

> 上面两张表里「任务进行中 / 任务完成 / 需要确认 · 单选 / 多选」都是**动图**。dsh 停下来等你选分支（`ask_user_question`）或审批计划（`exit_plan_mode`）时进入「需要你介入」态——**DeepSeek 与 HARNESS 分别向上下让开、开关上的白圆点放大成一个大圆角方形、里面浮现蓝色符号**，随后不停循环，**直到你在 dsh 页面做好选择、dsh 继续运算**，才播放**反向动画**收回成开关：
