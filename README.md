...
| 关闭 | 运行 · 空闲 | 任务进行中 | 任务完成 | 需要确认 | 等你选择 |
|:--:|:--:|:--:|:--:|:--:|:--:|
| ![关闭](docs/icon-off.png) | ![运行](docs/icon-running.png) | ![进行中](docs/icon-busy.png) | ![完成](docs/icon-remind-complete.png) | ![需确认](docs/icon-remind-confirm.png) | ![等你选择](docs/icon-remind-question.gif) |
| 红 · 滑块在左 | 绿 · 滑块在右 | 多色极光流动 | 边缘内发光 + 白色流光（绿） | 右上角红点脉冲 | 右上角红点脉冲 |

> 最后一格是**动图**：dsh 停下来等你选分支（`ask_user_question`）或审批计划（`exit_plan_mode`）时就进入「等你选择」态，图标右上角的红点持续脉冲，直到你回到页面或点「知道了」。「需要确认」（审批）用同一套红点脉冲——两者都表示**需要你介入**，具体是哪一种看程序坞角标（`!` 还是 `?`）。
...
- **动态提醒**：任务完成 / 需要确认 / **等你选择**（选项分支、计划审批）时，图标先弹跳 4 次 + 通知 + 提示音，随后转为**安静的持续提醒**；等你**回到 dsh 页面**、点图标或菜单「知道了」即停止（不会周期性乱跳）。
- **提示音可配置**（右键菜单 →「提示音」）：五种用途（需确认 / 等你选择 / 任务完成 / 服务启动 / 服务停止）各自显示当前音效名，可单独换音效（系统音效 + `~/Library/Sounds`）、可单独**试听**；三种提醒还能各自设置**重复次数**：`1` / `2` / `3` / `5` 次，或**「直到鼠标移到程序坞」**（鼠标进入程序坞或菜单栏区域即停，无需辅助功能权限）。设置持久化保存，另有「选择音效时试听」总开关。
