...
| 关闭 | 运行 · 空闲 | 任务进行中 | 任务完成 | 需要确认 | 等你选择 · 单选 | 等你选择 · 多选 |
|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| ![关闭](docs/icon-off.png) | ![运行](docs/icon-running.png) | ![进行中](docs/icon-busy.gif) | ![完成](docs/icon-remind-complete.gif) | ![需确认](docs/icon-remind-confirm.gif) | ![等你选择](docs/icon-remind-question.gif) | ![多选](docs/icon-remind-multi.gif) |
| 红 · 滑块在左 | 绿 · 滑块在右 | 多色极光流动 | 边缘内发光 + 白色流光（绿） | 问号变形动画（角标 `!`） | 问号变形动画（角标 `?`） | 对勾淡化叠加（角标 `?`） |

> 上表中「任务进行中 / 任务完成 / 需要确认 / 等你选择」都是**动图**。dsh 停下来等你选分支（`ask_user_question`）或审批计划（`exit_plan_mode`）时进入「等你选择」态——**DeepSeek 与 HARNESS 分别向上下让开、开关上的白圆点放大成一个大圆角方形、里面浮现蓝色符号**，随后不停循环，**直到你在 dsh 页面做好选择、dsh 继续运算**，才播放**反向动画**收回成开关：
> - **单选 / 普通提问**：圆内是蓝色**问号**，循环「亮度呼吸」（圆内蓝光一明一暗）。
> - **多选**：圆内是蓝色**对勾**，循环「淡化叠加」——对勾从起点写出 → 画完从起点逐渐褪掉 → 还剩约一半时新一笔从起点重画，两笔同框。
> - **需要确认**（审批）用同一套问号动画，与「等你选择」的区别看程序坞角标（`!` 还是 `?`）。
...
- **状态图标（4 态）**：
  ...
  - `待关注提醒`：**需要你介入**（需确认 / 等你选择）= 右上角**红点脉冲**（圆点缩放 + 柔和扩散，60fps）；**任务完成** = 绿色边缘内发光 + 边缘内侧 2px 白色流光
- **动态提醒**：需要确认 / **等你选择**时播放**问号变形动画**（0.5 秒变形 → 亮度呼吸一直循环 → 你做好选择后 0.42 秒反向收回成开关），并伴随弹跳 + 通知 + 提示音；任务完成仍用绿色边缘发光。点图标、选「知道了」或把鼠标移到程序坞也会立即收回。
...
- **完成数角标**：任务完成时在程序坞图标上显示**已完成计数**（`1`、`2`…）；回到 dsh 页面、点图标或菜单「知道了」后清零。需确认显示 `!`、等你选择显示 `?`。
