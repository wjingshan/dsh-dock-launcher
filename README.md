...
| 红 · 滑块在左 | 绿 · 滑块在右 | 多色极光流动 | 边缘内发光 + 白色流光（绿） | 问号变形动画 | 问号变形动画 |

> 最后一格是**动图**：dsh 停下来等你选分支（`ask_user_question`）或审批计划（`exit_plan_mode`）时进入「等你选择」态——**DeepSeek 与 HARNESS 分别向上下让开、开关上的白圆点放大成一个大正圆、圆内浮现蓝色问号**，随后问号不停循环「亮度呼吸」（圆内蓝光一明一暗），**直到你在 dsh 页面做好选择、dsh 继续运算**，才播放**反向动画**收回成开关。「需要确认」（审批）用同一套动画，两者的区别看程序坞角标（`!` 还是 `?`）。
